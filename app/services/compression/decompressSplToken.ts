...
export const buildDecompressSplTokenTx = async (payer: string, mintAddress: string, inputAccounts: ParsedTokenAccount[]): Promise<Transaction> => {
    try {
        ...
        const amountGroups = inputAccounts.reduce((acc, account) => {
        ...
        for (const [amount, accounts] of Object.entries(amountGroups)) {
            const decompressIx = await CompressedTokenProgram.decompress({
                payer: new PublicKey(payer), // The payer of the transaction.
                inputCompressedTokenAccounts: accounts, // compressed token accounts of payer.
